...
def read_datasets(path):
    ...
    train_x = df.ix[:train_size, :-1].values
    train_y = df.ix[:train_size, -1].values
    valid_x = df.ix[train_size:train_size + test_size, :-1].values
    valid_y = df.ix[train_size:train_size + test_size, -1].values
    test_x = df.ix[train_size + test_size:, :-1].values
    test_y = df.ix[train_size + test_size:, -1].values
...
def train_dbn(dbn, path, finetune_lr=0.01, pretraining_epochs=0,
              pretrain_lr=0.01, k=1, training_epochs=5, batch_size=1000):
    """
    ...
    :type path: string
    :param path: path the the pickled dataset
    :type batch_size: int
    :param batch_size: the size of a minibatch
    """
    ...
    print "Train set shape:", train_set_x.get_value(borrow=True).shape
    # compute number of minibatches for training, validation and testing
    ...
    improvement_threshold = 0.999  # a relative improvement of this
    # much is considered significant
    validation_frequency = min(n_train_batches, patience / 2)
    # go through this many minibatches before checking the network
    # on the validation set; in this
    # case we check every epoch
